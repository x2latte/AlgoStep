...
class BranchAndBoundKnapsack(KnapsackSolver):
    def run(self) -> Iterator[Tuple[str, List[bool], int, int]]:
        ...
        best_value = 0
        best_taken = [False] * n
        yield "Начало метода ветвей и границ", best_taken, 0, self.capacity
        ...
        def dfs(idx, weight, value, taken):
            ...
            if idx == n:
                if value > best_value:
                    ...
                    yield f"Новая лучшая ветка: ценность {best_value}", best_taken, best_value, self.capacity - weight
                return
            # не брать
            ...
            if weight + sorted_items[idx].weight <= self.capacity:
                ...
                yield f"Ветка: берём {sorted_items[idx].name}, ценность = {new_value}", [False]*n, new_value, self.capacity - new_weight
                yield from dfs(idx+1, new_weight, new_value, new_taken)

        yield from dfs(0, 0, 0, [])
        yield f"Ветви и границы завершены. Оптимум = {best_value}", best_taken, best_value, self.capacity - sum(self.items[i].weight for i in range(n) if best_taken[i])
