...
class DPKnapsack(KnapsackSolver):
    def run(self) -> Iterator[Tuple[str, List[bool], int, int]]:
        ...
        yield f"Начало динамического программирования, таблица {n}x{cap}", [], 0, cap
        for i in range(1, n+1):
            for w in range(cap+1):
                ...
                if w % max(1, cap//10) == 0 or w == cap:
                    yield f"DP: заполнена ячейка [{i}][{w}] = {dp[i][w]}", [], dp[i][w], cap - w
        ...
        yield f"DP завершён. Оптимальная ценность = {dp[n][cap]}", taken, dp[n][cap], w
